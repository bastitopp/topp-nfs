...
def submit(card_id):
    try:
        ...
        else: # Standard Flashcard / Case Study
...
@login_required
def exam_index():
    # Alle Typen erlauben, die automatisch prüfbar sind
    ...
    prepared = []
    exam_vars = {} # Variablen für Berechnungen
    ...
    for card in questions:
        ...
        if card.type == 'mc': 
            ...
            # Pool erstellen: Alle Items aus allen Gruppen in eine Liste
            temp_pool = []
            for gr in opts:
...
def exam_submit():
    score = 0
    # card_ids werden vom Formular gesendet, um die Reihenfolge/Vollständigkeit zu haben
    ...
    for cid in card_ids:
        try:
            ...
            if card.type == 'mc':
                if request.form.get(f'q_{cid}') == card.answer: is_correct = True
                ...
                try:
                    uo = json.loads(request.form.get(f'q_{cid}', '[]'))
                ...
                try:
                    ud = json.loads(request.form.get(f'q_{cid}', '{}'))
