...
def check_user_status():
    if current_user.is_authenticated:
        # 1. Zuletzt online aktualisieren
        if current_user.last_active is None or current_user.last_active < datetime.utcnow() - timedelta(minutes=5):
            current_user.last_active = datetime.utcnow()
            db.session.commit()

        # 2. ZUGANGSSPERRE FÜR ABGELAUFENE ACCOUNTS
        if request.endpoint:
            # Erlaube grundlegende Endpunkte, die immer funktionieren müssen
            is_auth_route = request.endpoint.startswith('auth.')
            is_allowed_route = request.endpoint in ['main.profile', 'static', 'main.service_worker']
            
            # Wenn die Route NICHT auf der Whitelist steht und der Nutzer keinen Zugang hat -> Sperren!
            if not (is_auth_route or is_allowed_route):
                if not current_user.has_access():
                    flash('Dein Zugang ist abgelaufen. Bitte schalte Premium frei, um weiterzulernen.', 'danger')
